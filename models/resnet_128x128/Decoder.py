...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.conv1T_stride = 2
        self.conv1T = nn.ConvTranspose2d(64, 1, kernel_size=7, stride=self.conv1T_stride, padding=3, bias=False)
    ...
    def _forward_impl(self, x, resnet_maxpool_indices):
        ...
        x = self.relu(x)
        x = self.conv1T(x, output_size=[self.conv1T_stride * x.shape[2], self.conv1T_stride * x.shape[3]])
        x = self.bn1(x)
        return x
    def forward(self, x):
